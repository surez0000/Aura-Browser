...
| PaneArea               | `components/PaneArea.tsx`                                                                          | the page area: one card, or 2–4 panes with headers and draggable dividers; measures each pane and reports the rectangles                       |
| PeekCard               | `components/PeekCard.tsx`                                                                          | link preview in a floating card; header sits around the measured page region, Open as Tab promotes it                                          |
| MiniApp                | `MiniApp.tsx`                                                                                      | the mini window's whole chrome: back, title, Open in Aura, close, and the measured page region                                                 |
...
| HistoryPanel           | `components/HistoryPanel.tsx`                                                                      | history manager (⌘Y) over a page snapshot: day groups, search, per-visit and per-site delete, clear by range                                   |
| ExtensionsPanel        | `components/ExtensionsPanel.tsx`                                                                   | extensions manager: list, on/off, remove, add from a folder or the store, and the store-install opt-in                                         |
